...
const numeroEmpresa = "5511985906431";
...
// ---- Integração com o backend ai-agent (Claude) ----
...
const STORAGE_KEY = "deividvilela_chat";
const loadStored = () => {
  try {
...
const tipoInteresseFrase = (t) => {
  const x = t.toLowerCase();
  if (x.includes("comprar")) return "comprar um imóvel";
  if (x.includes("vender")) return "vender um imóvel";
  if (x.includes("anunciar")) return "anunciar um imóvel para locação";
  if (x.includes("alugar")) return "alugar um imóvel";
  if (x.includes("avalia")) return "uma avaliação de imóvel";
  return `interesse: ${t}`;
};
...
const composeLeadMessage = (fields) => {
  ...
  const frases = ["Olá, Deivid!"];
  frases.push(nome ? `Meu nome é ${nome} e vim pelo chat do site.` : "Vim pelo chat do site.");

  if (interesse) {
    const tipo = tipoInteresseFrase(interesse);
    frases.push(`Tenho interesse em ${tipo}.`);
  }

  if (tamanho) frases.push(`O imóvel tem aproximadamente ${tamanho} m².`);
...
const buildWhatsappUrl = (originalUrl, leadFields) => {
  ...
  if (fields && fields.length > 0) {
    ...
    texto = "Olá, Deivid! Vim pelo chat do site e gostaria de conversar sobre um imóvel.";
    ...
    chatbotQuestion.textContent =
      chatState.returning && nome
        ...
        : chatState.returning
          ...
          : "Olá! Sou o assistente do Deivid Vilela. Quer comprar, vender ou alugar um imóvel?";
  }
  chatbotInput?.focus();
};
  ...
  try {
    ...
    if (whatsappMatch) {
      ...
      const cleanText = reply
        ...
        .replace(/\s{2,}/g, " ")
        .trim();
      const message = addChatMessage(cleanText || "Vou te encaminhar para o WhatsApp do Deivid.", "bot");
    ...
    const fallback = addChatMessage(
      "Tive um problema para responder agora. Você pode falar direto com o Deivid pelo WhatsApp.",
      "bot error"
    );
...
// ---- Imóveis dinâmicos (alimentados pelo painel Sua Imobiliária) ----
//
// Os imóveis cadastrados no painel são gravados na aba "Imoveis" da planilha
// do Google. Aqui lemos essa aba (somente leitura, via endpoint gviz) e
// montamos os cards. Basta a planilha estar compartilhada como "Qualquer
// pessoa com o link pode ver". Se a leitura falhar ou não houver imóveis,
// mantemos os cards de exemplo que já estão no HTML.
const IMOVEIS_CONFIG = {
  SHEET_ID: "1Z1-IkTqozPVzWIdQiZt-rbIxF0XaDfOVn33WxEkY-QI",
  SHEET_NAME: "Imoveis"
};

// Status de imóvel que NÃO devem aparecer no site público.
const IMOVEL_STATUS_OCULTOS = new Set(["vendido", "locado"]);

const escapeHtmlImovel = (str) =>
  String(str ?? "").replace(/[&<>"']/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]));

// Formata o preço como moeda; acrescenta "/mês" quando é locação.
const formatPreco = (valor, finalidade) => {
  const n = Number(String(valor).replace(/[^\d]/g, ""));
  if (!valor || Number.isNaN(n) || n === 0) return "Sob consulta";
  const base = n.toLocaleString("pt-BR", { style: "currency", currency: "BRL", maximumFractionDigits: 0 });
  return /loca|alug/i.test(String(finalidade)) ? `${base}/mês` : base;
};
...
// Lê a aba de imóveis via gviz e devolve uma lista de objetos.
const fetchImoveis = async () => {
  const url = `https://docs.google.com/spreadsheets/d/${IMOVEIS_CONFIG.SHEET_ID}/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
    IMOVEIS_CONFIG.SHEET_NAME
  )}`;
  ...
  const valueByLabel = (cells, label) => {
  ...
  return rows
    .map((row) => {
      const cells = row.c || [];
      return {
        id: valueByLabel(cells, "id"),
        titulo: valueByLabel(cells, "titulo"),
        tipo: valueByLabel(cells, "tipo"),
        finalidade: valueByLabel(cells, "finalidade"),
        status: valueByLabel(cells, "status"),
        preco: valueByLabel(cells, "preco"),
        bairro: valueByLabel(cells, "bairro"),
        cidade: valueByLabel(cells, "cidade"),
        quartos: valueByLabel(cells, "quartos"),
        banheiros: valueByLabel(cells, "banheiros"),
        vagas: valueByLabel(cells, "vagas"),
        area: valueByLabel(cells, "area"),
        foto: valueByLabel(cells, "foto"),
        descricao: valueByLabel(cells, "descricao")
      };
    })
    .filter((p) => p.titulo && !IMOVEL_STATUS_OCULTOS.has(String(p.status).toLowerCase()));
};

// Monta o HTML de um card de imóvel no mesmo formato dos exemplos estáticos.
const imovelCardHtml = (p) => {
  const finalidade = /loca|alug/i.test(p.finalidade) ? "Locação" : "Venda";
  const local = [p.tipo, [p.bairro, p.cidade].filter(Boolean).join(", ")].filter(Boolean).join(" · ");
  const specs = [
    ...
    .filter(Boolean)
    .map((s) => `<li>${escapeHtmlImovel(s)}</li>`)
    .join("");
  const foto = p.foto && /^https?:\/\//i.test(p.foto) ? p.foto : fotoPorTipo(p.tipo);
  const preco = formatPreco(p.preco, p.finalidade);
  const msg = `Olá, Deivid! Tenho interesse no imóvel "${p.titulo}"${local ? ` (${local})` : ""}.`;
  const whatsapp = `https://wa.me/${numeroEmpresa}?text=${encodeURIComponent(msg)}`;

  return `<article class="property-card">
      <img src="${escapeHtmlImovel(foto)}" alt="${escapeHtmlImovel(p.titulo)}" loading="lazy" />
      <div class="property-info">
        <div class="property-heading">
          <span class="property-tag">${finalidade}</span>
          ${local ? `<span class="property-location">${escapeHtmlImovel(local)}</span>` : ""}
        </div>
        <h3>${escapeHtmlImovel(p.titulo)}</h3>
        ${specs ? `<ul class="property-specs">${specs}</ul>` : ""}
      </div>
      <div class="property-cta">
        <strong class="property-price">${escapeHtmlImovel(preco)}</strong>
        <a class="button ghost small" href="${escapeHtmlImovel(whatsapp)}" target="_blank" rel="noopener noreferrer">Falar sobre este imóvel</a>
      </div>
    </article>`;
};
...
const carregarImoveis = async () => {
  ...
  try {
    ...
    // Falha na leitura (planilha privada, offline etc.): preserva o fallback.
    console.warn("Não foi possível carregar os imóveis da planilha:", error?.message || error);
  }
};
